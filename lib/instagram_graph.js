...
class InstagramGraphService {
  ...
  // Auto-discover linked Instagram Business Account ID
  async getActiveId() {
    if (this.isSimulated) return null;
    if (this.discoveredId) return this.discoveredId;

    // Se o ID configurado parecer ser o System User ou inválido, tenta encontrar o ID correto nas páginas
    if (this.businessAccountId === '61589554145653' || !this.businessAccountId.startsWith('178')) {
      console.log("[InstagramGraphService] ID configurado parece ser o System User. Iniciando auto-descoberta...");
      try {
        const result = await this.makeRequest('me/accounts?fields=instagram_business_account{id,username,name}');
        if (result && result.data && result.data.length > 0) {
          for (const page of result.data) {
            if (page.instagram_business_account) {
              this.discoveredId = page.instagram_business_account.id;
              console.log(`[InstagramGraphService] Auto-descoberto Instagram Business Account ID: ${this.discoveredId} (${page.instagram_business_account.username}) da Página: ${page.name}`);
              return this.discoveredId;
            }
          }
        }
      } catch (e) {
        console.error("[InstagramGraphService] Erro ao tentar descobrir ID:", e.message);
      }
    }
  ...
  async getProfile(forceSimulated = false) {
    ...
    const data = await this.makeRequest(`${activeId}?fields=${fields}`);
  ...
  async getMedia(forceSimulated = false) {
    ...
    const activeId = await this.getActiveId();
    const result = await this.makeRequest(`${activeId}/media?fields=${fields}`);
  ...
  async publishPost(imageUrl, caption) {
    ...
    const activeId = await this.getActiveId();
    const container = await this.makeRequest(`${activeId}/media`, {
      method: 'POST',
      body: JSON.stringify({
        image_url: imageUrl,
        caption: caption
    ...
    const publish = await this.makeRequest(`${activeId}/media_publish`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  async getMessages() {
    ...
    const activeId = await this.getActiveId();
    const result = await this.makeRequest(`${activeId}/conversations?fields=participants,messages{message,from,timestamp}`);
    const conversations = result.data || [];

    return conversations.map(c => {
      const contact = c.participants?.data?.find(p => p.id !== activeId) || {};
      const msgList = (c.messages?.data || []).reverse().map(m => ({
        sender: m.from?.username || m.from?.name || (m.from?.id === activeId ? 'euefrai' : 'client'),
        text: m.message,
